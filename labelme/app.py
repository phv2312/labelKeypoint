...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None, output=None):
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=open_file, close=close, next=next,
                ...
                fileMenuActions=(open_file,next,save,saveAs,close,quit),
                beginner=(), advanced=(),
        ...
        addActions(self.menus.file,
                   (open_file, self.menus.recentFiles, saveAs, close, None, quit))
        ...
        self.actions.beginner = (
            open_file, save, None, create, copy, delete, None,
            zoomIn, zoom, zoomOut, fitWindow, fitWidth)

        self.actions.advanced = (
            open_file, next, delete, None,
            createMode, editMode, next_person, prev_person, None,
            hideAll, showAll)
    ...
    def addLabel(self, shape, text):
        item = QListWidgetItem(shape.label)
        item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
        item.setCheckState(Qt.Checked)
        self.itemsToShapes.append((item, shape))
        self.labelList.addItem(item)
        self.labelDict['person_' + str(self.person_id)][text] = item
    ...
    def saveFile(self, _value=False):
        ...
        if self.hasLabels():
            imgid = self.imglist[self.imgCnt].split('.')[0]
